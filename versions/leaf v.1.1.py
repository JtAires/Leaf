...
if not os.path.exists(PASTA_BACKUP): # <--- Garante que a pasta backup existe
...
class AppOrganizacao:
    def __init__(self, root):
        self.root = root
        self.root.title("Leaf v.1.1")
        self.root.geometry("1000x550")
        
        try:
            # Agora usamos o caminho_recurso para encontrar o ícone dentro do .exe!
            self.root.iconbitmap(caminho_recurso('icone.ico'))
        except Exception as e:
            print("Erro ao carregar ícone principal:", e)
    ...
    def iniciar_sistema_backup(self):
            # Cria um nome de pasta com a data de hoje (ex: 2026-06-05)
            hoje_str = datetime.now().strftime("%Y-%m-%d")
            pasta_hoje = os.path.join(PASTA_BACKUP, hoje_str)

            # Só copia os arquivos se o backup de hoje ainda não existir
            if not os.path.exists(pasta_hoje):
                os.makedirs(pasta_hoje)
                
                arquivos_json = [ARQUIVO_PENDENTES, ARQUIVO_CONCLUIDAS, ARQUIVO_DIARIO, ARQUIVO_RECORRENTES]
                
                for arquivo in arquivos_json:
                    if os.path.exists(arquivo):
                        # shutil.copy2 copia o arquivo e preserva suas propriedades originais
                        shutil.copy2(arquivo, pasta_hoje)

            # --- O GATILHO DA MEIA-NOITE ---
            agora = datetime.now()
            amanha = agora + timedelta(days=1)
            # Define o alvo para 00:00:05 de amanhã (5 segs de margem de segurança)
            proxima_meia_noite = datetime(amanha.year, amanha.month, amanha.day, 0, 0, 5)
            
            # Calcula quantos milissegundos faltam para a meia-noite
            ms_restantes = int((proxima_meia_noite - agora).total_seconds() * 1000)
            
            # O Tkinter agenda a própria função para rodar de novo na virada do dia
            self.root.after(ms_restantes, self.iniciar_sistema_backup)
    ...
    def configurar_interface_principal(self):
        ...
        # --- SUBSTITUIR AS DUAS LINHAS DO lbl_titulo_tabela POR ISTO: ---
        frame_titulo_busca = tk.Frame(self.root)
        frame_titulo_busca.pack(fill=tk.X, padx=15, pady=(5, 0))

        lbl_titulo_tabela = tk.Label(frame_titulo_busca, text="📌 To-do", font=("Arial", 11, "bold"), fg="#166534")
        lbl_titulo_tabela.pack(side=tk.LEFT)

        # Barra de pesquisa do To-do
        self.entry_busca_todo = tk.Entry(frame_titulo_busca, width=30)
        self.entry_busca_todo.pack(side=tk.RIGHT)
        self.entry_busca_todo.bind("<KeyRelease>", lambda e: self.atualizar_tabela_principal())
        ...
        tk.Label(frame_legenda, text=" Essa semana ", bg="#4ade80", fg="#064e3b", font=("Arial", 8)).pack(side=tk.LEFT, padx=2)
        tk.Label(frame_legenda, text=" Semana que vem ", bg="#86efac", fg="#064e3b", font=("Arial", 8)).pack(side=tk.LEFT, padx=2)
        tk.Label(frame_legenda, text=" Esse mês ", bg="#dcfce7", fg="#064e3b", font=("Arial", 8)).pack(side=tk.LEFT, padx=2)
        tk.Label(frame_legenda, text=" Depois desse mês ", bg="#e2e8f0", fg="#0f172a", font=("Arial", 8)).pack(side=tk.LEFT, padx=2)
        ...
        self.tabela.tag_configure('essa_semana', background='#4ade80', foreground='#064e3b')
        self.tabela.tag_configure('semana_que_vem', background='#86efac', foreground='#064e3b')
        self.tabela.tag_configure('esse_mes', background='#dcfce7', foreground='#064e3b')
        self.tabela.tag_configure('depois_desse_mes', background='#e2e8f0', foreground='#0f172a')
        self.tabela.tag_configure('sem_data', background='#ffffff', foreground='#000000')

        self.tabela.pack(fill=tk.BOTH, expand=True, padx=10, pady=(2, 10))

        self.menu_contexto = tk.Menu(self.root, tearoff=0)
        self.menu_contexto.add_command(label="➔ Agendar para hoje", command=self.adicionar_ao_dia)
        ...
        self.menu_contexto.add_command(label="Gerenciar palavras-chave", command=lambda: self.gerenciar_palavras_chave_popup(self.tabela, self.pendentes, ARQUIVO_PENDENTES))
...
# --- ATALHOS DE TECLADO ---
        ...
        self.root.bind('<Control-h>', self.adicionar_ao_dia)
        self.root.bind('<Control-H>', self.adicionar_ao_dia)
    ...
    def atualizar_tabela_principal(self):
            ...
            dias_para_sabado_atual = (5 - hoje.weekday()) % 7
            data_sabado_atual = hoje + timedelta(days=dias_para_sabado_atual)
            data_sabado_que_vem = data_sabado_atual + timedelta(days=7)
            
            # Pega o texto da busca principal (se a barra já existir)
            busca = self.entry_busca_todo.get().lower() if hasattr(self, 'entry_busca_todo') else ""
            ...
            for ativ in self.pendentes:
                ...
                # Se a palavra buscada não estiver no título nem nas tags, pula para a próxima
                if busca not in texto_atividade and busca not in tags:
                    continue
                # -----------------------
                ...
                if prazo != datetime.max:
                    ...
                    if prazo_date < hoje:
                        ...
                        tag_linha = 'hoje'
                    elif prazo_date <= data_sabado_atual:
                        tag_linha = 'essa_semana'
                    elif prazo_date <= data_sabado_que_vem:
                        tag_linha = 'semana_que_vem'
                    elif prazo_date.month == hoje.month and prazo_date.year == hoje.year:
                        tag_linha = 'esse_mes'
                    else:
                        tag_linha = 'depois_desse_mes'
                
                str_tags = ", ".join(ativ.get('keywords', []))
                self.tabela.insert("", tk.END, values=(ativ['id'], ativ['atividade'], ativ['deadline'], ativ.get('comentarios', ''), str_tags), tags=(tag_linha,))
    ...
    def adicionar_ao_dia(self, event=None):
        selecao = self.tabela.selection()
        if not selecao:
    ...
    def abrir_concluidas(self):
        ...
        self.menu_contexto_conc.add_command(label="Gerenciar palavras-chave", command=lambda: self.gerenciar_palavras_chave_popup(self.tabela_conc, self.concluidas, ARQUIVO_CONCLUIDAS))
    ...
    def gerenciar_palavras_chave_popup(self, tabela_alvo, lista_dados, arquivo_alvo):
        ...
        item = tabela_alvo.item(selecao[0])
        id_ativ = item['values'][0]
        
        ativ_idx = next((i for i, x in enumerate(lista_dados) if x.get('id') == id_ativ), None)
        ...
        def add_tag(event=None):
            ...
            if nova_tag and nova_tag not in ativ['keywords']:
                ...
                # Salva dinamicamente no arquivo certo
                salvar_dados_seguros(arquivo_alvo, lista_dados)
                
                # Atualiza dinamicamente a tabela certa
                if tabela_alvo == self.tabela:
                    self.atualizar_tabela_principal()
                elif hasattr(self, 'tabela_conc') and tabela_alvo == self.tabela_conc:
                    self.atualizar_tabela_concluidas()
    ...
    def abrir_menu_dia(self):
        ...
        # As abas começam com títulos genéricos, mas serão atualizadas dinamicamente pelos contadores
        ...
        for tb in (tb_hoje, tb_antigas):
            tb.tag_configure('alta', background='#fca5a5', foreground='black')      
            tb.tag_configure('media', background='#fde047', foreground='black')     
            tb.tag_configure('baixa', background='#86efac', foreground='black')     
            tb.tag_configure('normal', background='#dcfce7', foreground='#14532d')  
            tb.bind("<Button-1>", self.desmarcar_clique_vazio)
        
        # --- FUNÇÃO DE CARREGAMENTO COM ORDENAÇÃO E CONTADORES (Pontos 1 e 2) ---
        def carregar_listas():
            ...
            # Separa os dados em listas temporárias antes de renderizar
            for ativ in self.pendentes:
                if ativ.get('data_dia'):
                    if ativ['data_dia'] == hoje_str:
                        lista_hoje.append(ativ)
                    else:
                        lista_antigas.append(ativ)
            
            # Ponto 1: Lógica de Ordenação Inteligente (Prioridade -> Prazo mais próximo)
            pesos_prioridade = {"Alta": 0, "Média": 1, "Baixa": 2}
            def chave_ordenacao(x):
                peso = pesos_prioridade.get(x.get('prioridade', ''), 3)
                prazo_dt = converter_data(x.get('deadline', ''))
                return (peso, prazo_dt)
            
            lista_hoje.sort(key=chave_ordenacao)
            lista_antigas.sort(key=chave_ordenacao)
            
            # Insere os itens ordenados na tabela "Para hoje"
            for ativ in lista_hoje:
            ...
            # Insere os itens ordenados na tabela "Antigas"
            for ativ in lista_antigas:
                pri = ativ.get('prioridade', '')
                tag_linha = 'alta' if pri == "Alta" else 'media' if pri == "Média" else 'baixa' if pri == "Baixa" else 'normal'
                valores = (ativ['id'], ativ['atividade'], ativ['deadline'], ativ.get('comentarios', ''))
                tb_antigas.insert("", tk.END, values=valores, tags=(tag_linha,))
            
            # Ponto 2: Atualização dos Contadores Dinâmicos nas abas do Notebook
            notebook.tab(0, text=f"Para hoje ({len(lista_hoje)})")
            notebook.tab(1, text=f"Atividades não concluídas ({len(lista_antigas)})")
                        
        carregar_listas()
        
        # --- MENU DE CONTEXTO REFORMULADO (Ponto 4) ---
        ...
        # Ponto 4: Função para promover a atividade antiga alterando a data_dia
        def promover_para_hoje():
            ...
            salvar_dados_seguros(ARQUIVO_PENDENTES, self.pendentes)
            carregar_listas() # Atualiza e reordena tudo instantaneamente
            messagebox.showinfo("Sucesso", f"{len(selecao)} atividade(s) movida(s) para hoje!", parent=janela)

        
# --- Ações rápidas para o menu de contexto ---
        def concluir_pelo_menu():
            if tabela_alvo[0]:
                self.concluir_atividade(tabela_alvo[0])
                carregar_listas() # Atualiza os contadores das abas na hora

        def remover_pelo_menu():
            if tabela_alvo[0]:
                self.remover_do_dia(tabela_alvo[0])
                carregar_listas() # Atualiza os contadores das abas na hora

        # --- Montagem do menu de contexto ---
        menu_ctx.add_command(label="✓ Concluir selecionada(s)", command=concluir_pelo_menu)
        menu_ctx.add_command(label="❌ Remover do dia", command=remover_pelo_menu)
        menu_ctx.add_separator()
        menu_ctx.add_command(label="🎯 Adicionar/editar prioridade", command=abrir_popup_prioridade)
        menu_ctx.add_command(label="➔ Trazer para Hoje", command=promover_para_hoje)
        ...
        def capturar_clique_direito(event):
            ...
            if item_sob_mouse:
                ...
                # Só ativa o "Trazer para Hoje" se estiver na aba de Antigas
                if tabela == tb_antigas:
                    menu_ctx.entryconfig("➔ Trazer para Hoje", state=tk.NORMAL)
                else:
                    menu_ctx.entryconfig("➔ Trazer para Hoje", state=tk.DISABLED)
        ...
        for r_idx, semana in enumerate(cal_matriz):
            for c_idx, dia in enumerate(semana):
                if dia == 0:
                    ...
                    # Verifica se a célula atual é exatamente o dia de hoje
                    eh_hoje = (dia == hoje.day and self.mes_cal == hoje.month and self.ano_cal == hoje.year)
                    
                    # Sistema de cores priorizando o dia de hoje
                    if eh_hoje:
                        cor_bg = "#16a34a"  # Verde escuro (mesma cor da tag "Hoje" da tabela)
                        cor_fg = "white"    # Texto branco para dar contraste
                    elif tem_tarefa:
                        cor_bg = "#bbf7d0"  # Verde claro para dias com tarefa
                        cor_fg = "black"
                    else:
                        cor_bg = "#f1f5f9"  # Fundo padrão
                        cor_fg = "black"
                    
                    txt_botao = f"{dia}*" if tem_diario else str(dia)
                    font_estilo = ("Arial", 9, "bold") if (tem_tarefa or tem_diario or eh_hoje) else ("Arial", 9)
                    
                    # Adicionamos o fg=cor_fg para o texto ficar branco caso seja hoje
                    btn_dia = tk.Button(self.frame_dias_cal, text=txt_botao, width=5, bg=cor_bg, fg=cor_fg, font=font_estilo,
...
# =================================================================
    ...
    def nova_atividade_popup(self, event=None):
        ...
        # Criamos um frame para colocar o Entry e o Botão lado a lado
        frame_deadline = tk.Frame(popup)
        frame_deadline.pack(pady=5)
        
        var_deadline = tk.StringVar()
        # A largura do Entry foi levemente reduzida para caber o botão
        entry_deadline = tk.Entry(frame_deadline, width=32, textvariable=var_deadline)
        entry_deadline.pack(side=tk.LEFT, padx=(0, 5))
        self.setup_mascara_data(entry_deadline, var_deadline)

        # Função que cria e gerencia o mini-calendário flutuante
        def abrir_seletor_data():
            ...
            hoje_sel = datetime.today()
            ano_sel = tk.IntVar(value=hoje_sel.year)
            mes_sel = tk.IntVar(value=hoje_sel.month)

            frame_nav = tk.Frame(top)
            frame_nav.pack(pady=10)

            lbl_mes_ano = tk.Label(frame_nav, font=("Arial", 10, "bold"), width=12)
            
            frame_dias = tk.Frame(top)
            frame_dias.pack()

            def desenhar_mini_cal():
                for w in frame_dias.winfo_children():
                    w.destroy()
                
                meses_nomes = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"]
                lbl_mes_ano.config(text=f"{meses_nomes[mes_sel.get()-1]} {ano_sel.get()}")

                dias_semana = ["D", "S", "T", "Q", "Q", "S", "S"]
                for i, d in enumerate(dias_semana):
                    tk.Label(frame_dias, text=d, font=("Arial", 8, "bold"), width=3).grid(row=0, column=i)
                
                cal_obj = calendar.Calendar(firstweekday=6) # Começando no domingo
                cal_matrix = cal_obj.monthdayscalendar(ano_sel.get(), mes_sel.get())

                for r_idx, semana in enumerate(cal_matrix):
                    for c_idx, dia in enumerate(semana):
                        if dia != 0:
                            # Destaca o dia atual em verde
                            eh_hoje = (dia == hoje_sel.day and mes_sel.get() == hoje_sel.month and ano_sel.get() == hoje_sel.year)
                            bg_color = "#16a34a" if eh_hoje else "#f1f5f9"
                            fg_color = "white" if eh_hoje else "black"
                            
                            btn = tk.Button(frame_dias, text=str(dia), width=2, bg=bg_color, fg=fg_color, relief="flat",
                                            command=lambda d=dia: selecionar(d))
                            btn.grid(row=r_idx+1, column=c_idx, padx=1, pady=1)

            def selecionar(dia):
                var_deadline.set(f"{dia:02d}/{mes_sel.get():02d}/{ano_sel.get()}")
                top.destroy()
                entry_comentarios.focus_set() # Joga o foco para o próximo campo automaticamente

            def mes_ant():
                m = mes_sel.get() - 1
                if m < 1:
                    mes_sel.set(12)
                    ano_sel.set(ano_sel.get() - 1)
                else:
                    mes_sel.set(m)
                desenhar_mini_cal()
            ...
            tk.Button(frame_nav, text="<", command=mes_ant, relief="flat", bg="#e2e8f0").pack(side=tk.LEFT)
            lbl_mes_ano.pack(side=tk.LEFT, padx=5)
            tk.Button(frame_nav, text=">", command=mes_prox, relief="flat", bg="#e2e8f0").pack(side=tk.LEFT)

            desenhar_mini_cal()

        # Botão que chama o mini-calendário
        btn_cal = tk.Button(frame_deadline, text="📅", command=abrir_seletor_data, bg="#e2e8f0", relief="groove")
        btn_cal.pack(side=tk.LEFT)
        # --- FIM DA SUBSTITUIÇÃO ---
